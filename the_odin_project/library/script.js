...
function addBooktoTable() {
  ...
  <td>${
    condition
      ? '<i title="click to change status" class="fas fa-check-circle statu"></i>'
      : '<i title="click to change status" class="fas fa-times-circle statu"></i>'
  }</td>
...
function removeItem() {
  ...
  Object.entries(remove).forEach((icon) => {
    ...
    value.addEventListener("click", () => {
      library = library.filter((el, index) => {
        return index != key;
      });
      addBooktoTable();
    });
